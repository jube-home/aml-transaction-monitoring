...
namespace Jube.Test.Service.EntityAnalysisModelTtlCounter
{
    ...
    public sealed class EntityAnalysisModelTtlCounterServiceTests(DatabaseFixture fx) : IAsyncLifetime
    {
        ...
        [Fact]
        public async Task GetByEntityAnalysisModelIdReturnsRowsForThatModelOrderedByIdAsync()
        {
            await using var dbContext = fx.GetDbContext();
            ...
            var savedA1 = await service.InsertAsync(NewDto(modelAId, UniqueName("ByModelA1")));
            createdIds.Add(savedA1.Id);
            var savedA2 = await service.InsertAsync(NewDto(modelAId, UniqueName("ByModelA2")));
            ...
            forModelA.Should().Contain(d => d.Id == savedA1.Id);
            forModelA.Should().Contain(d => d.Id == savedA2.Id);
            forModelA.Should().NotContain(d => d.Id == savedB.Id);
            forModelA.Select(d => d.Id).Should().BeInAscendingOrder();
        }

        [Fact]
        public async Task GetByEntityAnalysisModelGuidReturnsRowsForThatModelAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var modelAId = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
        ...
        [Fact]
        public async Task NegativeTtlCounterValueIsRejectedAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var modelId = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            var dto = NewDto(modelId, UniqueName("NegativeValue"));
            dto.TtlCounterValue = -1;

            var ex = await Assert.ThrowsAsync<DtoValidationException>(() => service.InsertAsync(dto));
            ex.Result.Errors.Should().Contain(e => e.ErrorCode == "TtlCounterValueInvalid");
        }
        ...
        [Fact]
        public async Task MissingTtlCounterDataNameIsRejectedAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var modelId = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            var dto = NewDto(modelId, UniqueName("NoDataName"));
            dto.TtlCounterDataName = null;

            var ex = await Assert.ThrowsAsync<DtoValidationException>(() => service.InsertAsync(dto));
            ex.Result.Errors.Should().Contain(e => e.ErrorCode == "TtlCounterDataNameRequired");
        }

        [Fact]
        public async Task SumEnabledWithoutDataValueIsRejectedButNotRequiredWhenSumDisabledAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var modelId = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);

            var withoutSum = NewDto(modelId, UniqueName("NoSum"));
            var savedWithoutSum = await service.InsertAsync(withoutSum);
            createdIds.Add(savedWithoutSum.Id);

            var withSum = NewDto(modelId, UniqueName("SumNoValue"));
            withSum.EnableSum = true;

            var ex = await Assert.ThrowsAsync<DtoValidationException>(() => service.InsertAsync(withSum));
            ex.Result.Errors.Should().Contain(e => e.ErrorCode == "TtlCounterDataValueRequired");
        }
