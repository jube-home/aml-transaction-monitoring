...
namespace Jube.App.Endpoints
{
    ...
    public static class EntityAnalysisModelEndpoints
    {
        private const string Base = "/api/EntityAnalysisModel";

        public static void MapEntityAnalysisModelEndpoints(this IEndpointRouteBuilder endpoints)
        {
            var group = endpoints.MapGroup(Base)
                .RequireAuthorization()
                .WithTags("EntityAnalysisModel");

            group.MapGet("", GetAsync)
                .Produces<List<EntityAnalysisModelDto>>()
                .WithName("EntityAnalysisModelGetAll");

            group.MapGet("{id:int}", GetByIdAsync)
                .Produces<EntityAnalysisModelDto>()
                .WithName("EntityAnalysisModelGetById");

            group.MapPost("", CreateAsync)
                .Produces((int)HttpStatusCode.OK)
                .Produces<ValidationResult>((int)HttpStatusCode.BadRequest)
                .WithName("EntityAnalysisModelCreate");

            group.MapPut("", UpdateAsync)
                .Produces((int)HttpStatusCode.OK)
                .Produces<ValidationResult>((int)HttpStatusCode.BadRequest)
                .Produces((int)HttpStatusCode.NoContent)
                .WithName("EntityAnalysisModelUpdate");

            group.MapDelete("{id:int}", DeleteAsync)
                .Produces((int)HttpStatusCode.OK)
                .Produces((int)HttpStatusCode.NoContent)
                .WithName("EntityAnalysisModelDelete");
        }

        private static async Task<IResult> GetAsync(
            HttpContext httpContext, ILog log, DynamicEnvironment dynamicEnvironment,
            ...
            IServiceChangeBus serviceChangeBus, CancellationToken token)
        {
            ...
            try
            {
                var service = await EntityAnalysisModelService.CreateAsync(dbContext, user, log, stringLocalizerFactory,
        ...
        private static async Task<IResult> GetByIdAsync(
            int id, HttpContext httpContext, ILog log, DynamicEnvironment dynamicEnvironment,
            IStringLocalizerFactory stringLocalizerFactory,
            IServiceChangeBus serviceChangeBus, CancellationToken token)
        {
            var user = httpContext.User.Identity?.Name;
        ...
        private static async Task<IResult> CreateAsync(
            [FromBody] EntityAnalysisModelDto model, HttpContext httpContext, ILog log,
            DynamicEnvironment dynamicEnvironment,
            IStringLocalizerFactory stringLocalizerFactory, IServiceChangeBus serviceChangeBus, CancellationToken token)
        {
            var user = httpContext.User.Identity?.Name;
            if (log.IsDebugEnabled)
            {
