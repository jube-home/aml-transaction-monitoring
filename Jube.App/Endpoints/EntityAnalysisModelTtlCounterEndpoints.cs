...
namespace Jube.App.Endpoints
{
    public static class EntityAnalysisModelTtlCounterEndpoints
    {
        ...
        public static void MapEntityAnalysisModelTtlCounterEndpoints(this IEndpointRouteBuilder endpoints)
        {
            ...
            group.MapGet("ByEntityAnalysisModelId/{id:int}", GetByEntityAnalysisModelIdAsync)
                .Produces<List<EntityAnalysisModelTtlCounterDto>>()
        ...
        {
            var user = httpContext.User.Identity?.Name;
            if (log.IsDebugEnabled) log.Debug($"GET {Base}/ByEntityAnalysisModelId/{id}: entry user={user}");
            ...
            try
            {
                var service = await EntityAnalysisModelTtlCounterService.CreateAsync(dbContext, user, log,
                    stringLocalizerFactory, serviceChangeBus, token);
                var result = await service.GetByEntityAnalysisModelIdAsync(id, token);
                return TypedResults.Ok(result);
            }
            catch (NotAuthenticatedException)
            {
                if (log.IsWarnEnabled)
                    log.Warn($"GET {Base}/ByEntityAnalysisModelId/{id}: 403 (not authenticated) user={user}");

                return TypedResults.Forbid();
            }
            catch (ForbiddenException)
            {
                if (log.IsWarnEnabled) log.Warn($"GET {Base}/ByEntityAnalysisModelId/{id}: 403 user={user}");
            ...
            catch (OperationCanceledException)
            {
                if (log.IsDebugEnabled)
                    log.Debug($"GET {Base}/ByEntityAnalysisModelId/{id}: client cancelled user={user}");

                throw;
            }
            catch (Exception e)
            {
                log.Error($"GET {Base}/ByEntityAnalysisModelId/{id}: 500 user={user}", e);
                return TypedResults.StatusCode((int)HttpStatusCode.InternalServerError);
            }
        }
        ...
        {
            var user = httpContext.User.Identity?.Name;
            if (log.IsDebugEnabled) log.Debug($"GET {Base}/ByEntityAnalysisModelGuid/{guid}: entry user={user}");
            ...
            {
                if (log.IsWarnEnabled) log.Warn($"GET {Base}/ByEntityAnalysisModelGuid/{guid}: 403 user={user}");
