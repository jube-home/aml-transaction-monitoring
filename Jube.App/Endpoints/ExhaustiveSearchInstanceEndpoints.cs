...
namespace Jube.App.Endpoints
{
    public static class ExhaustiveSearchInstanceEndpoints
    {
        ...
        {
            var user = httpContext.User.Identity?.Name;
            if (log.IsDebugEnabled) log.Debug($"PUT {Base}/stop/{guid}: entry user={user}");
            ...
            {
                if (log.IsWarnEnabled) log.Warn($"PUT {Base}/stop/{guid}: 403 (not authenticated) user={user}");

                return TypedResults.Forbid();
            }
            catch (ForbiddenException)
            {
                if (log.IsWarnEnabled) log.Warn($"PUT {Base}/stop/{guid}: 403 user={user}");

                return TypedResults.Forbid();
            }
            catch (OperationCanceledException)
            {
                if (log.IsDebugEnabled) log.Debug($"PUT {Base}/stop/{guid}: client cancelled user={user}");
